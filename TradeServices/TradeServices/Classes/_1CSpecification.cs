...
namespace TradeServices.Classes
{
    public class _1CSpecification : _1CUtManager, IResultArray<Specification>
    {
        ...
        private const string _1CQuery = @"ВЫБРАТЬ
                                            ...
                                            ИЗ
                                                РегистрСведений.СпецификацияНоменклатуры  КАК СпецификацияНоменклатурыСрезПоследних
                                            внутреннее  соединение Маршрут	м по м.ТорговаяТочка = СпецификацияНоменклатурыСрезПоследних.ТорговаяТочка
                                            внутреннее соединение (выбрать максимум(ПЕриод) Период,ТорговаяТочка из  РегистрСведений.СпецификацияНоменклатуры 
						                                            сгруппировать по ТорговаяТочка ) Периоды
		                                            по Периоды.ТорговаяТочка = СпецификацияНоменклатурыСрезПоследних.ТорговаяТочка 
		                                            и Периоды.Период = СпецификацияНоменклатурыСрезПоследних.Период";

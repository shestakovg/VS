         ...
         private const string _1CQuery = @"выбрать ном.ссылка как номенклатурнаяГруппа   
                                                ...
                                                где ном.Ссылка в иерархии (
                                                    ...
                                                    где бп.Ссылка в (выбрать БрендКПРодаже из
                                                        Справочник.МаршрутыТорговыхПредставителей где ссылка =&Маршрут ))
                                            ...
                                            выбрать т.ТорговаяТочка OutletId,т.Номенклатура SkuId,НАЧАЛОПЕРИОДА(т.Дата, ДЕНЬ) LastDate, 
		                                            выбор когда Товары.Склад.Наименование = ""Розница"" Тогда 2 иначе 1 конец Warehouse,
                                                    Товары.КоличествоУпаковок as Qty из Таб т
                                            внутреннее соединение    Документ.ЗаказКлиента.Товары Товары
                                                по т.ТорговаяТочка =  Товары.Ссылка.ТорговаяТочка
                                                и  т.Дата = Товары.Ссылка.Дата
                                                и т.Номенклатура = Товары.Номенклатура
                                                И т.Номенклатура.Родитель в  (выбрать номенклатурнаяГруппа из Бренды)";
        #endregion
        ...
        public ClientCardSku[] ConvertToArray(IQueryable<System.Data.DataRow> queryable = null)
        {
            ...
            IEnumerable<ClientCardSku> result =
                    ...
                    select new ClientCardSku()
                    {
                        ...
                        Warehouse = Convert.ToInt32(row["Warehouse"])
                    };

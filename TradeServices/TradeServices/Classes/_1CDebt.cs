    ...
    public class _1CDebt : _1CUtManager, IResultArray<DebtData>
    {
        ...
        private const string _1CQuery = @"ВЫБРАТЬ РАЗЛИЧНЫЕ
                                                ...
                                                ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                                                ...
	                                                ВЫБОР
		                                                КОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&Период,ДЕНЬ,0), ДЕНЬ) >= выбор когда ЕСТЬNULL(РасчетыСКлиентамиОстатки.ЗаказКлиента.ДатаПлатежа, &Период)=ДАТАВРЕМЯ(1, 1, 1) then &Период else ЕСТЬNULL(РасчетыСКлиентамиОстатки.ЗаказКлиента.ДатаПлатежа, &Период) end 
			                                                ТОГДА РасчетыСКлиентамиОстатки.СуммаОстаток
		                                                ИНАЧЕ 0
	                                                КОНЕЦ КАК ПросроченныйДолг,
	                                                РАЗНОСТЬДАТ(выбор когда ЕСТЬNULL(РасчетыСКлиентамиОстатки.ЗаказКлиента.ДатаПлатежа, &Период)=ДАТАВРЕМЯ(1, 1, 1) then &Период else ЕСТЬNULL(РасчетыСКлиентамиОстатки.ЗаказКлиента.ДатаПлатежа, &Период) end , ДОБАВИТЬКДАТЕ(&Период,ДЕНЬ,0), ДЕНЬ) КАК ДнейПросрочки,
	                                                //ВЫБОР
